...
## Second hosted run: bounded dispatch and behavioral refusal grading

CI run 35038909108, downloaded to `/tmp/symphony-ci-eval2.2SHy28`, verifies that the weak-root dry-run case now passes all eleven graders. The registration smoke still fails: seven Agent invocations, four spawned/completed children, and a final report naming both primary and recovery leads. The follow-up investigation identified an invalid `automatic:small` initial assessment and correction attempts; the retained judge evidence shows the recovery launch and final multiple-lead report. The mismatch case again passes all six zero-action guards but fails the exact-text regex; its actual final response is still absent from the uploaded reports.

The fixed smoke now explicitly requires project profile `small` and run mode `small`, with the actual-run-id `small:small` receipt supplied to the assessor. `automatic` is explicitly forbidden as a size. Its hard budget is two Agent/Task invocation attempts, including denied and resume calls; correction, retry, result collection, recovery, and replacement-lead calls are forbidden. Background collection uses only TaskOutput. Invalid or missing child evidence must be reported as failure without inventing a receipt or launching another agent. The lead receives the same-run assessment-receipt requirement up front, so ordinary reassessment does not require a recovery child. The exact-two tool grader and strict registration trace grader are unchanged.

The mismatch grader is now an LLM trace rubric requiring actual skill invocation, trusted runtime comparison before project action, both declared and actual model names, refusal, and an instruction to start a correctly configured task. It allows effort to be explicitly unavailable or unverified. It rejects affirmative continuation, lifecycle advice, project work, invented runtime evidence, missing observations, and adversarial examples that merely mention the mismatch. This removes punctuation/line-break dependence without treating the absent original response as a pass. The six zero-action graders and CI threshold 0.9 remain unchanged.

Test-first checks failed on the missing fixed receipt/budget and the old regex grader. The updated declaration tests preserve the exact-two bound, require the strict positive and negative refusal criteria, require all six zero-action guards, and reject any remaining mismatch regex definition. All eleven declaration/JavaScript checks, the existing adversarial `automatic:small` hook regression, plugin validation, and diff checks pass. The full suite passed all 133 tests in 18.071 seconds. The old refusal-specific JavaScript examples were removed with that regex; all remaining regex graders still compile in JavaScript. Fresh hosted evaluation remains required; local early-access gating has not changed.
