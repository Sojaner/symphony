...
9. Provider hook fixtures are captured from real installed sessions of the pinned provider versions, by `scripts/capture_fixtures.py`, and committed under `tests/fixtures/captured/`. Every payload field the reducer or adapter depends on must appear in a captured fixture; a field absent from both the provider documentation and the captures may not be relied on. Events not yet captured are listed in the conformance test rather than assumed, so the gap stays visible. Claude `SessionStart`, `UserPromptSubmit` and `Stop` are captured; the spawn events on both providers and every Codex event are not.
