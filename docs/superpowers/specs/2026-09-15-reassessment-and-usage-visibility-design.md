...
Agent records gain an optional `usage` object. Claude Agent `PostToolUse` token values are explicitly scoped final-request observations (`final_request_*` fields, source, and scope), not whole-agent totals. Its `duration_ms` and `tool_uses` describe the whole agent run. It may contain only host-reported token counters and breakdowns, duration, tool-use count, observation time, source, and scope. Missing numeric values remain absent internally and render as exactly `not exposed by host`.
...
`/symphony:agents` adds final-request total tokens, input, output, cache creation/read, run duration, run tool uses, and usage source/token scope. `/symphony:agents --all` retains the same columns for historical runs. `/symphony:status` reports observed final-request tokens plus the count of agents with unreported final-request totals; it labels the result partial and never presents it as a complete agent total.

## Usage collection

The shared hook accepts usage only from host event fields associated with a known agent id. For Claude, a `PostToolUse` hook scoped to the Agent tool records `totalTokens` and the `usage` breakdown as final-request observations, while `totalDurationMs` and `totalToolUseCount` describe the whole agent run. Background launches (`status: async_launched`) remain unreported until a later host event supplies an observation. A malformed breakdown does not discard valid top-level counters.
