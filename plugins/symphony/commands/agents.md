...
Report the injected Symphony agent ledger as a compact table with run, id, status, role, model, effort, total/input/output/cache-creation/cache-read tokens, duration, tool uses, and usage source. When the host exposes a live agent-listing tool, reconcile live status and metadata before reporting. Preserve `not exposed by host`; never infer model, effort, usage, or cost.
