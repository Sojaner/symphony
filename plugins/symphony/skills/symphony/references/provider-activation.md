...
- On Windows, if `/hooks` shows a trusted hook but this project's heartbeat is absent, check `where.exe python`, `python --version`, and whether Windows PowerShell permits the packaged script in the environment that launches Codex. The first `python.exe` on `PATH` must launch; a later working copy cannot rescue a broken WindowsApps alias. Restart Codex, review the changed hook, and submit a prompt. An interpreter or PowerShell launch failure happens before Symphony can write a heartbeat or fault record; inspect the host hook failure.
- `--dangerously-bypass-hook-trust` is for already-vetted automation, never normal onboarding.

If an upgrade removes a versioned cache while agents are running, a command captured by the old Codex session can fail before Symphony starts. Inspect the host agent roster and collect any available results. Restart Codex, review the new hook in `/hooks`, and run `$symphony:symphony status` in the new session. A missing `SubagentStop` leaves the outcome unknown; a new heartbeat does not reconstruct it. Preserve a run owned by another live session. After confirming the tracked agents have ended, `$symphony:symphony stop --force` can archive the run and retain their IDs as unreconciled; it does not mark their work completed. Prefer finishing active agents before upgrading so their stop hooks can run from the loaded version.
