...
Bind `<run-id>` to the current run. The current run's root must relay both lines in an owner control response and wait for the Stop hook to persist the accepted assessment before execution. Immediately end that response after the registration and assessment lines for a synchronous assessor. Never spawn the lead in the same response. After the hook confirms acceptance, emit `Completed: <agent id/role> — <status>` and append token or duration segments only for values the host exposed, then announce `Mode: <mode> — <strategy> — <reason>` using the strategy and reason from the hook acknowledgment, then continue.
