...
2. **Classify.** Treat controls as inert. Send substantive or uncertain work to a bounded `strongest/high` assessor; skip assessment only when classification is deterministic. Read [role contracts](references/role-contracts.md) before constructing any assessment or work packet.
...
6. **Complete.** Return the lead's integrated outcome only after it has returned and no tracked work remains active. Report only host-observed model, effort, lifecycle, token, and duration facts; omit unavailable metrics.
