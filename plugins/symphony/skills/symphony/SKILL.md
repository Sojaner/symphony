...
1. Emit `Delegating: assessor [<model>/<effort>] — pending — <bounded objective>` before spawning exactly one `symphony_assessor` with no inherited turns, using the injected strongest-available general reasoning model profile at `high`. Give the Codex spawn the task name `symphony_assessor__<model-slug>__<effort-slug>` or start the Claude description with `symphony_assessor [<model>/<effort>]:`. If `high` is unavailable, use that model's highest available effort. If no stronger child can be spawned, fail closed and explain which capability is missing; do not pretend a weak root is protected.
...
Make every role identifiable in both providers. Codex `task_name` is `symphony_<role>__<model-slug>__<effort-slug>`. Claude `description` starts `symphony_<role> [<model>/<effort>]:`. Workers and reviewers follow the same rule with a short role slug. When trusted runtime metadata exposes the root identity, prefix its progress updates with `thin orchestrator [<model>/<effort>]:`; otherwise use `thin orchestrator:` and omit unavailable values.

After every observed start or completion, before every blocking wait, and in the final response, replay one cumulative snapshot derived from the lifecycle ledger:

```text
Delegation log:
- Delegating: assessor [<model>/<effort>] — <agent-id> — <objective>
- Completed: assessor [<model>/<effort>] — <agent-id> — <status>
- Delegating: lead [<model>/<effort>] — <agent-id> — <objective>
- Waiting: lead [<model>/<effort>] — <agent-id> — active
```

Include every observed assessor, lead, worker, and reviewer in lifecycle order. Replace `Waiting:` with `Completed:` when terminal. Never emit a standalone `Waiting:` update: replaying the snapshot prevents providers that collapse or replace commentary from hiding earlier delegations. Append token or duration segments only when exposed, and omit unavailable measurements rather than describing them as unavailable.

The user must be able to see why a run costs what it costs. After the hook accepts an assessment, the root announces `Mode: <mode> — <strategy> — <reason>`: small means the lead executes directly, medium means the lead plus at most two bounded independent workers, and large means dependency-aware waves. The lead ends its result with `Delegation summary:` listing each worker as `<agent id> — <model>/<effort> — <assigned job> — <outcome>`, or `Delegation summary: none`. The root's final report carries the cumulative `Delegation log:` and one `Routing:` line: `Routing: mode <mode> (<strategy>) — <reason>; assessor <agent id> <model>/<effort>; lead <agent id> <model>/<effort>; workers <agent id> <model>/<effort> — <assigned job>, … or none`. The Stop hook offers copy-ready ledger values. A placeholder does not complete the run. The hook verifies the snapshot, mode and strategy, exact assessor and lead identity/model/effort, and every observed worker against the ledger.
...
3. Make the root-final response self-contained: repeat the integrated deliverable, the cumulative `Delegation log:`, both assessor and lead `Completed: <agent-id>/<role> — <status>` records with each agent's observed terminal status, one `Routing:` line with the mode strategy and every agent's actual model/effort and assigned job, a `Verification:` line with authoritative evidence, the selected mode, and the exact completion receipt. Add token or duration segments only when exposed.
