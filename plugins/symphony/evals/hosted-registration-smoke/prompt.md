...
description: Verifies lifecycle and role assignment, not actual child model/effort; real Agent calls and owner-root registration/assessment relays without project writes.
...
Set the assessor Agent/Task `model` explicitly to `opus` and the lead model explicitly to `sonnet`; never inherit the weak root model. Set each `description` to start with `symphony_assessor [opus/high]:` or `symphony_lead [sonnet/medium]:` for its assigned role.

Require exactly two successfully started and completed distinct children, one assessor and one lead. Guard-denied attempts that start no child are not extra children. Retry a blocked lead attempt only after the required lifecycle handoff is accepted; never launch another child once the lead has started. Complete within this case's 12-turn, 300-second limit. Never use Agent/Task for correction, result collection, or recovery. Use only `TaskOutput` for collecting a background result. If a child result or required receipt is invalid or missing, report the failure plainly without another agent invocation or a success claim. Do not spawn a replacement lead.
...
Give the first child the explicit assignment `symphony_assessor`, the injected run id, objective, acceptance criteria, project profile, lifecycle status, and the skill's absolute routing-reference paths. This fixed empty-workspace contract task has project profile `small` and run mode `small`; `automatic` describes the profile source, never a valid project size. Require the assessor's concise assessment to end with exactly `SYMPHONY_ASSESSMENT:<actual-run-id>:small:small`, `SYMPHONY_ASSESSMENT_REASON:<single bounded line>`, and `SYMPHONY_LEAD_ROUTE:<actual-run-id>:sonnet:medium:none:0`, substituting only the actual injected run id and a factual reason. Both size fields must be `small`, never `automatic`. After it returns, relay those actual lines with `SYMPHONY_REGISTER:<run-id>:assessor:<agent-id>` in an owning-root final-channel control response, without a run-completion receipt. End that response and wait for the hook's accepted-assessment acknowledgment before the second launch. Do not invent or correct an assessment receipt in the root.

Give the separate `symphony_lead` the accepted assessment and bounded deliverable. Require its result to include the two-paragraph deliverable and its own unchanged same-run three-line assessment receipt for any ordinary reassessment relay. If the synchronous call returns the terminal result and host id together, include `SYMPHONY_REGISTER:<run-id>:lead:<agent-id>` in the self-contained final report; do not spend a separate acknowledgment turn. If the host instead launches it in the background, register that id immediately in a final-channel response with no run-completion receipt, wait for the hook acknowledgment, then collect that same lead's terminal result with `TaskOutput`. Relay only its actual receipt if the hook requires ordinary reassessment. Never claim a registration or receipt was accepted without hook evidence; report any missing capability or evidence plainly.

Keep the whole run compact so lifecycle evidence is retained. Before each wait and in the final response, replay the cumulative `Delegation log:` so earlier dispatches remain visible. The final response must be self-contained and repeat, in order: both distinct actual agent ids and roles in that log; both `Completed: <agent-id>/<role> — <status>` summary records; one `Routing:` line with the mode strategy and each agent's actual model/effort, with no `<...>` placeholders; a `Verification:` line; the hook-confirmed small assessment mode; the two-paragraph deliverable; exactly `<!-- SYMPHONY_MODE:small -->`; and the exact injected completion receipt. Do not use a prose `mode: small` substitute.
