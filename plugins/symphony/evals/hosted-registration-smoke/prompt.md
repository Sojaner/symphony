...
Visibly announce each delegation before its actual Agent/Task call and its completion afterward, using the skill's full records and honest host usage fields. Request synchronous calls (`run_in_background: false` when exposed). If the host still backgrounds a child, register the returned id in a final-channel control response without run completion, then use its blocking result tool (`TaskOutput` with `block: true` when exposed) for that same id. Do not launch another child to collect a result. Agent and Task are host aliases; neither a denied attempt nor a retry counts as a successful launch.
