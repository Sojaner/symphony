...
Require exactly two successfully started and completed distinct children, one assessor and one lead. Allow at most one guard-denied premature attempt that starts no child, followed by a single retry for the missing role after the required lifecycle handoff is accepted: at most three Agent/Task invocations total. A denied attempt is not a successful child. Never use Agent/Task for correction, result collection, or recovery; the guard-denied attempt is the only permitted retry. Use only `TaskOutput` for collecting a background result. If a child result or required receipt is invalid or missing, report the failure plainly without another agent invocation or a success claim. Do not spawn a replacement lead.
